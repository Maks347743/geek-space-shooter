...
public class Player extends Ship {
    ...
    public void update(float dt) {
        ...
        /* time += dt;
         for (int i = 0; i < 10; i++) {
             game.getParticleEmitter().setup(position.x + 32 * (float) Math.cos(time * 5 + i * 0.628f), position.y + 32 * (float) Math.sin(time * 5 + i * 0.628f), MathUtils.random(-10, 10), MathUtils.random(-10, 10), 0.2f, 0.8f, 1.1f, 1, 0, 0, 1, 1, 1, 1, 1);
    ...
    public void fire() {
        super.fire();
        fireSound.setVolume(fireSound.play(), 0.3f);
    }
}
